...
public class GirisEkrani extends JFrame {
	...
	private void girisYap() {
		...
		String gosterilecekKullaniciAdi = username;

		String sql = "SELECT * FROM users WHERE username = ? AND password = ? AND role = ?";

		try {
			String sifrelenmisUsername = veriTabani.sifrele(username);
			String hashlenmisPassword = veriTabani.getInstance().hashPassword(password);

			PreparedStatement pstmt = veriTabani.getInstance().getConnection().prepareStatement(sql);

			pstmt.setString(1, sifrelenmisUsername);
			pstmt.setString(2, hashlenmisPassword);
			pstmt.setString(3, role);

			ResultSet rs = pstmt.executeQuery();

			if (rs.next()) {
				Session.aktifKullanici = gosterilecekKullaniciAdi;
			...
			ex.printStackTrace();
			lblMessage.setText("Veritabanı hatası!");
		} catch (Exception ex) {
			ex.printStackTrace();
			lblMessage.setText("Giriş sırasında hata oluştu!");
		}
	}
}
